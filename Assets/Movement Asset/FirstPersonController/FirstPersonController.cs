...
public class FirstPersonController : MonoBehaviour
{
    ...
    public static Vector3 savePosition = new Vector3(135.9f, 2.3f, 11.9f);
    ...
    public int numCockroachesLeft;

    [SerializeField] GameObject popup;
    public void SetPopupText(string s)
    {
        popup.GetComponent<TextMeshProUGUI>().text = s;
    }
    ...
    private void Awake()
    {
        ...
        transform.position = savePosition;
        Debug.Log("spawn pos: " + transform.position);
    ...
    private void Update()
    {
        ...
        if (enableZoom)
        {
            ...
            if(holdToZoom && !isSprinting)
            {
                ...
                {
                    isZoomed = true;
                    playerArms.transform.localPosition = new Vector3(-0.49f, 0, 0.14f);
                    playerArms.transform.localRotation = Quaternion.Euler(new Vector3(0, 102.165f, -2.219f));
                }
                else if(Input.GetKeyUp(zoomKey))
                {
                    isZoomed = false;
                    playerArms.transform.localPosition = Vector3.zero;
                    playerArms.transform.localRotation = Quaternion.Euler(new Vector3(0, 90, -3.699f));
                }
            }
            ...
            if(isZoomed)
            {
                playerCamera.fieldOfView = Mathf.Lerp(playerCamera.fieldOfView, 30, zoomStepTime * Time.deltaTime);
            }
            else if(!isZoomed && !isSprinting)
            {
                playerCamera.fieldOfView = Mathf.Lerp(playerCamera.fieldOfView, 60, zoomStepTime * Time.deltaTime);
            }
        }
    ...
    public void Die()
    {
        playerCanMove = false;
        cameraCanMove = false;
        Health.instance.SetInvincibleState(true);
        playerShooter.SetShootingState(false);
    }
    public void Revive()
    {
        playerCanMove = true;
        cameraCanMove = true;
        Health.instance.SetInvincibleState(false);
        playerShooter.SetShootingState(true);
    }
